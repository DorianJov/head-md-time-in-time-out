...
void loop() {
  ...
  //delay(1) if some probleme
  unsigned long ms = millis();
  int deltaTime = ms - prevTime;
  float speed = deltaTime == 0 ? 0 : (Count_pulses - prevPulses) / deltaTime;
  prevPulses = Count_pulses;
  prevTime = ms;

  Serial.println(String(" isMoving ?  ") + isMoving(false) + String("\tIs stopped for 5 sec:  ") + isStopforXsec(5000) + String("\tResult:  ") + Count_pulses + "\tSpeed: " + deltaTime + String("\tCurrentLevel: ") + whichLevel);
